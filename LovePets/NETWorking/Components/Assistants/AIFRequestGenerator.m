...
//    NSString *signature = [AIFSignatureGenerator signPostWithApiParams:requestParams privateKey:service.privateKey publicKey:service.publicKey];
    ...
    NSString *signature = [AIFSignatureGenerator signPostWithApiParams:requestParams privateKey:service.privateKey publicKey:service.publicKey];
    NSString *urlString = [NSString stringWithFormat:@"%@%@", service.apiBaseUrl, methodName];
    NSMutableDictionary *dcit = [[NSMutableDictionary alloc]initWithDictionary:requestParams];
    [dcit setObject:signature forKey:@"sig"];
    NSURLRequest *request = [self.httpRequestSerializer requestWithMethod:@"POST" URLString:urlString parameters:dcit error:NULL];
    request.requestParams = dcit;
    [AIFLogger logDebugInfoWithRequest:request apiName:methodName service:service requestParams:dcit httpMethod:@"POST"];
